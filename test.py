...
def validation(model, criterion, evaluation_loader, converter, opt):
    ...
    with torch.no_grad():
        for i, (image_tensors, labels) in tqdm(enumerate(evaluation_loader), total=len(evaluation_loader),
            ...
            print("Before calculating accuracity")
            # Optimized accuracy & confidence score calculation
            preds_prob = F.softmax(preds, dim=2)
            preds_max_prob, _ = preds_prob.max(dim=2)
            confidence_score_list = []

            for gt, pred, pred_max_prob in zip(labels, preds_str, preds_max_prob):
                # Prune after "[s]" token if present
                ...
                # Case insensitive comparison (if applicable)
                if opt.sensitive and opt.data_filtering_off:
                    pred = pred.lower()
                    gt = gt.lower()
                    pred = re.sub(r'[^0-9a-z]', '', pred)
                    gt = re.sub(r'[^0-9a-z]', '', gt)

                if pred == gt:
                    n_correct += 1

                # ICDAR2019 Normalized Edit Distance using Levenshtein
                if len(gt) == 0 or len(pred) == 0:
                    norm_ED += 0
                else:
                    norm_ED += 1 - Levenshtein.distance(pred, gt) / max(len(gt), len(pred))

                # Calculate confidence score
                try:
                    confidence_score = pred_max_prob.cumprod(dim=0)[-1]
                except:
                    confidence_score = 0  # For empty pred case after pruning
                confidence_score_list.append(confidence_score)
            print("After calculating accuracity")
